...
from emblem.config import load_config
from emblem.runtime.state_provider import create_state_provider
...
STATE_PROVIDER = create_state_provider(CONFIG, STATE_FILE, MAP_FILE)


def load_snapshot(timeout: float | None = None) -> TurnSnapshot:
    bridge_cfg = CONFIG.get("bridge", {})
    if bridge_cfg.get("enabled"):
        if timeout is None:
            timeout = bridge_cfg.get("timeout_ms", 100) / 1000.0
    return STATE_PROVIDER.load_snapshot(timeout=timeout)
...
SIMULATION_MODE = (
    False  # Set to True to use fast simulation for RL, False for real BizHawk
)

# --- Utility: Identify good terrain tiles by symbol ---
GOOD_TERRAIN_SYMBOLS = {
    "F",
    "^",
    "0C",
    "0D",
    ...
    "C",
    "T",
}  # Forest, Thicket, Hill, Fort, Gate, Throne, etc.
GOOD_TERRAIN_MIN_DEF = (
    1  # Minimum defense bonus to consider a tile 'good' (for struct-based fallback)
)
...
def wait_for_state_update(prev_snapshot, timeout=3):
    ...
    while time.time() - start < timeout:
        try:
            ...
            continue
        if (
            snapshot.current_turn != prev_snapshot.current_turn
            or snapshot.turn_phase != prev_snapshot.turn_phase
        ):
            return snapshot
    return snapshot
...
def is_level_beaten(snapshot):
    ...
    return (not any(e.is_alive for e in snapshot.enemies)) and any(
        e.hidden_status != 0x20 for e in snapshot.enemies
    )
...
def end_turn_in_bizhawk(cursor_pos, snapshot):
    ...
    for unit in snapshot.units:
        if unit.id == 1 and (unit.can_act or not unit.has_acted):
            print(
                f"[END TURN BLOCKED] Eliwood (id=1) can still act (can_act={unit.can_act}, has_acted={unit.has_acted}, turn_status=0x{unit.turn_status:02X})."
            )
            return cursor_pos
    ...
    for _ in range(100):  # Up to 10 seconds
        ...
        if pos is not None:
            ...
            if new_pos != pos:
                print(
                    f"[END TURN] Enemy phase started or ended, cursor is movable at {new_pos}."
                )
                break
...
def move_cursor_to(target_pos, current_pos, max_attempts=3):
    if current_pos is None:
        print(
            f"[CURSOR] Warning: current_pos is None, defaulting to target_pos {target_pos}"
        )
...
def get_cursor_xy_from_state(state_file):
    ...
    if CONFIG.get("bridge", {}).get("enabled"):
        try:
            snapshot = load_snapshot()
            return snapshot.cursor_position
        except Exception as exc:  # pragma: no cover - defensive logging
            print(f"[ERROR] Failed to fetch cursor via bridge: {exc}")
            return None, None
    try:
        ...
        for line in lines:
            ...
            if in_realtime:
                ...
                if (
                    not line
                    or (
                        line.replace("_", "").isupper()
                        and line.replace("_", "").isalpha()
                    )
...
def perform_attack_action(action, cursor_pos, probe=False):
    print(
        f"[ACTION] {action.unit.name} moving to {action.target_position} to ATTACK {action.target_unit.name if action.target_unit else ''}{' [PROBE]' if probe else ''}"
    )
    # Move cursor to unit
    ...
    if pos_check != action.unit.position:
        print(
            f"[ERROR] Cursor not on expected unit {action.unit.name} at {action.unit.position}, but at {pos_check}. Attempting recovery."
        )
        return_to_map()
        cursor_pos = move_cursor_to(action.unit.position, cursor_pos)
        pos_check = get_cursor_position()
        if pos_check != action.unit.position:
            print(
                f"[FATAL] Still not on expected unit after recovery. Skipping action."
            )
    ...
    dist = abs(action.unit.position[0] - action.target_position[0]) + abs(
        action.unit.position[1] - action.target_position[1]
    )
    pos_check = get_cursor_position()
    if pos_check != action.target_position:
        print(
            f"[ERROR] Cursor not on expected target tile {action.target_position}, but at {pos_check}. Attempting recovery."
        )
        return_to_map()
        cursor_pos = move_cursor_to(action.target_position, cursor_pos)
        pos_check = get_cursor_position()
        if pos_check != action.target_position:
            print(
                f"[FATAL] Still not on expected target after recovery. Skipping action."
            )
            return cursor_pos if probe else (cursor_pos, None)
    # Confirm move
    press_key("x", duration=0.05)
    ...
    time.sleep(0.1)
    print(
        f"[ACTION] {action.unit.name} initiated attack at {action.target_position}. Waiting for battle to finish..."
    )
...
def execute_action_in_bizhawk(action, cursor_pos, prev_snapshot):
    print(
        f"[ACTION] {action.unit.name} preparing to {action.action_type} at {action.target_position}"
    )
    if action.action_type == "attack":
        cursor_pos = perform_attack_action(action, cursor_pos)
    elif action.action_type == "wait" or action.action_type == "move":
        print(
            f"[ACTION] {action.unit.name} moving to {action.target_position} to {action.action_type.upper()}"
        )
        cursor_pos = move_cursor_to(action.unit.position, cursor_pos)
        pos_check = get_cursor_position()
        if pos_check != action.unit.position:
        ...
        print(
            f"[ACTION] {action.unit.name} performing {action.action_type} at {action.target_position}"
        )
    ...
    def check_fn(snapshot):
        for u in snapshot.units:
            if u.id == action.unit.id and (
                u.position != action.unit.position or not u.can_act
            ):
                return True
        return False
    ...
    if is_menu_open():
        ...
        cursor_pos = get_cursor_position() or cursor_pos
    print(
        f"[RESULT] {action.unit.name} {action.action_type} to {action.target_position}"
    )
    cursor_pos = get_cursor_position() or cursor_pos
...
def log_state(snapshot):
    ...
    print(
        f"Alive Players: {sum(1 for u in snapshot.units if u.is_alive)} | Alive Enemies: {sum(1 for e in snapshot.enemies if e.is_alive)}"
    )
    print("Units left to act:")
    for u in snapshot.get_available_units():
        if not u.has_acted and u.can_act:
            print(
                f"  - {u.name} at {u.position} HP: {u.hp[0]}/{u.hp[1]} Status: {u.turn_status_text}"
            )


def compute_reward(
    prev_snapshot, curr_snapshot, action=None, level_beaten=False, player_dead=False
):
    reward = 0
    ...
    if action is not None:
        if action.action_type == "move":
            terrain = curr_snapshot.map.get_terrain_at(
    ...
    if (
        ...
        and action.unit.hp[0] < action.unit.hp[1] * 0.3
    ):
        for enemy in curr_snapshot.enemies:
            if (
                enemy.is_alive
                and abs(action.unit.position[0] - enemy.position[0])
                + abs(action.unit.position[1] - enemy.position[1])
                <= 2
            ):
                reward -= 10
                break
    ...
    if (
        action is not None
        and action.unit.position == action.target_position
        and not killed_enemy
    ):
        reward -= 10
    # Penalty if unit's turn_status did not change to 0x42 after action
    if action is not None:
        unit_after = next(
            (u for u in curr_snapshot.units if u.id == action.unit.id), None
        )
...
def filter_actions(actions, snapshot=None, movement_map=None, range_map=None):
    ...
    for a in actions:
        ...
        if (
            ...
            if (
                y >= len(movement_map)
                or x >= len(movement_map[y])
                or movement_map[y][x] == 0xFF
            ):
                continue
...
def probe_battle_struct_for_attack(
    attacker,
    defender,
    target_tile,
    state_file,
...
def score_tile_for_survivability(
    unit, x, y, snapshot, state_file, move_cursor_to, press_key, get_cursor_position
):
    ...
    for enemy in snapshot.enemies:
        ...
        if enemy_can_attack_tile(enemy, x, y, snapshot):
            battle_struct = probe_battle_struct_for_attack(
                enemy,
                unit,
                (x, y),
                state_file,
...
def probe_all_weapons_battle_structs(
    attacker,
    defender,
    target_tile,
    state_file,
    ...
    for slot_idx, (item_id, uses) in enumerate(attacker.items):
        if uses > 0:
            ...
            cursor_pos = get_cursor_position()
            cursor_pos, battle_struct = perform_attack_action(
                action, cursor_pos, probe=True
            )
    ...
    while True:
        ...
        while not done:
            ...
            for unit in initial_units:
                print(
                    f"[DEBUG] Probing unit: {unit.name} at {unit.position}, can_act={unit.can_act}, has_acted={unit.has_acted}"
                )
                ...
                if pos_check != unit.position:
                    print(
                        f"[ERROR] Cursor not on expected unit {unit.name} at {unit.position}, but at {pos_check}. Skipping unit."
                    )
                    continue
                press_key("x", duration=0.05)
                time.sleep(0.05)
                ...
                actions = coordinator.action_generator._generate_unit_actions(
                    unit, movement_map, range_map
                )
                non_wait_actions = [
                    a
                    for a in actions
                    if not (
                        a.action_type == "wait" and a.target_position == unit.position
                    )
                ]
                filtered_actions = filter_actions(
                    non_wait_actions, snapshot, movement_map, range_map
                )
                print(
                    f"[DEBUG] Filtered actions for {unit.name}: {[a.action_type for a in filtered_actions]}"
                )
                if filtered_actions:
            ...
            internal_enemies = [
                e for e in snapshot.enemies if e.is_alive and e.is_visible
            ]
            ...
            if all_no_attack:
                print(
                    "[INFO] No attack actions available for any unit. Ending player turn."
                )
                if not SIMULATION_MODE:
            ...
            while actionable_units:
                ...
                for unit, actions in zip(actionable_units, actionable_actions):
                    ...
                    for a in actions:
                        if a.action_type == "attack" and a.target_unit is not None:
                            if not any(
                                e.id == a.target_unit.id for e in internal_enemies
                            ):
                ...
                for idx, (unit, actions) in enumerate(
                    list(
                        zip(
                            attack_first_units + other_units,
                            attack_first_actions + other_actions,
                        )
                    )
                ):
                    ...
                    if attack_actions:
                        print(
                            f"[DEBUG] {unit.name} has {len(attack_actions)} attack actions available."
                        )
                        scored_attacks = []
                        probed_pairs = set()
                        for a in attack_actions:
                            ...
                            probe_key = (
                                a.target_position,
                                a.target_unit.id if a.target_unit else None,
                            )
                            ...
                            for aa in attack_actions:
                                if is_good_terrain(
                                    snapshot,
                                    aa.target_position[0],
                                    aa.target_position[1],
                                ):
                                    candidate_tiles.append(aa)
                            if 0 < len(candidate_tiles) <= GOOD_TERRAIN_MAX_PROBES:
                                # Only probe good tiles
                                if not is_good_terrain(
                                    snapshot, a.target_position[0], a.target_position[1]
                                ):
                                    continue  # Skip bad tiles
                            # Probe all weapons for this attack action
                            weapon_results = probe_all_weapons_battle_structs(
                                unit,
                                a.target_unit,
                                a.target_position,
                                STATE_FILE,
                            ...
                            for battle_struct, item_id, slot_idx in weapon_results:
                                if battle_struct:
                                    will_kill = (
                                        battle_struct["attack"] >= a.target_unit.hp[0]
                                        or battle_struct.get("cur_hp", 1) <= 0
                                    )
                                    will_die = (
                                        battle_struct["cur_hp"] <= 0
                                        or battle_struct["attack"] >= unit.hp[0]
                                    )
                                    score = 0
                                    ...
                                    scored_attacks.append(
                                        (score, weapon_action, will_kill)
                                    )
                                else:
                                    scored_attacks.append((0, a, False))
                        if not scored_attacks:
                            print(
                                f"[WARN] No valid attack actions for {unit.name}. Skipping unit."
                            )
                            continue
                        ...
                        print(
                            f"[DEBUG] {unit.name} has {len(actions)} non-attack actions available."
                        )
                        ...
                        if move_actions:
                            ...
                            for a in move_actions:
                                ...
                                for enemy in internal_enemies:
                                    dist = abs(
                                        a.target_position[0] - enemy.position[0]
                                    ) + abs(a.target_position[1] - enemy.position[1])
                            ...
                            if not scored_moves:
                                print(
                                    f"[WARN] No valid move actions for {unit.name}. Skipping unit."
                                )
                                continue
                            ...
                            if not actions:
                                print(
                                    f"[WARN] No available actions for {unit.name}. Skipping unit."
                                )
                                continue
                            if random.random() < epsilon:
                                ...
                                features = [
                                    coordinator.get_action_features(a) for a in actions
                                ]
                                scores = coordinator.neural_network.evaluate_actions(
                                    actions, features
                                )
                                best_idx = max(
                    ...
                    print(
                        f"[DEBUG] Chosen action for {unit.name}: {chosen_action.action_type} to {chosen_action.target_position}"
                    )
                    prev_snapshot = snapshot
                    if SIMULATION_MODE:
                        snapshot, reward = coordinator.simulate_action(
                            prev_snapshot, chosen_action
                        )
                        ...
                        if (
                            chosen_action.action_type == "attack"
                            and chosen_action.target_unit is not None
                            and chosen_will_kill
                        ):
                            internal_enemies = [
                                e
                                for e in internal_enemies
                                if e.id != chosen_action.target_unit.id
                            ]
                    else:
                        cursor_pos, snapshot = execute_action_in_bizhawk(
                            chosen_action, cursor_pos, prev_snapshot
                        )
                        reward = compute_reward(prev_snapshot, snapshot, chosen_action)
                        # --- Check if unit is marked as moved in new snapshot ---
                        acted_unit = next(
                            (u for u in snapshot.units if u.id == unit.id), None
                        )
                        if acted_unit and acted_unit.turn_status not in [0x02]:
                            print(
                                f"[WARN] Unit {unit.name} (id={unit.id}) not marked as moved after action. Forcibly marking as moved."
                            )
                        ...
                        if chosen_action.action_type == "attack" and (
                            not chosen_will_kill
                        ):
                    ...
                    replay_buffer.append(
                        (prev_snapshot, chosen_action, reward, snapshot)
                    )
        ...
        if episode % 10 == 0:
            try:
                ...
                print(
                    f"[REPLAY BUFFER] Saved to {REPLAY_BUFFER_PATH} at episode {episode}"
                )
            except Exception as e:
